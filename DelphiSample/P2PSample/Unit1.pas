...
type
  ...
  private
    ...
    procedure InviteIncoming(callbackIndex: NativeInt; sessionId: NativeInt;
      ...
      videoCodecs: PAnsiChar; existsAudio: Boolean; existsVideo: Boolean; sipMessage:PAnsiChar);
    ...
    procedure InviteSessionProgress(callbackIndex: NativeInt;
      ...
      existsEarlyMedia: Boolean; existsAudio: Boolean; existsVideo: Boolean; sipMessage:PAnsiChar);
    procedure InviteRinging(callbackIndex: NativeInt; sessionId: NativeInt;
      statusText: PAnsiChar; statusCode: integer; sipMessage:PAnsiChar);
    ...
    procedure InviteUpdated(callbackIndex: NativeInt; sessionId: NativeInt;
      ...
      existsVideo: Boolean; sipMessage:PAnsiChar);
   ...
   procedure onVideoDecoderCallback(var Message: TMessage); message WM_DECODER_VIDEOCALLBACK;
...
begin
  ...
  PostMessage(Form1.Handle, WM_RAW_VIDEOCALLBACK, 0, LPARAM(params));  // NativeUInt is for the new x64-compilers. All compilers before use integer
  result := 0;
...
function VideoDecoderCallback(obj: Pointer; sessionId, width, height, frameRate, bitrate: integer):integer; cdecl;
var
  params: TONVideoDecoderCallbackParams;
begin
  params := TONVideoDecoderCallbackParams.Create;
  params.sessionId := sessionId;
  params.width := width;
  params.height :=  height;
  ...
  PostMessage(Form1.Handle, WM_DECODER_VIDEOCALLBACK, 0, LPARAM(params));  // NativeUInt is for the new x64-compilers. All compilers before use integer
  result := 0;
...
procedure TForm1.InviteIncoming(callbackIndex: NativeInt; sessionId: NativeInt;
  ...
  existsAudio: Boolean; existsVideo: Boolean; sipMessage:PAnsiChar);
var
...
procedure TForm1.ReceivedSignaling(callbackIndex: NativeInt;
  sessionId: NativeInt; sipMessage: PAnsiChar);
var
...
begin
  ...
  params.callbackIndex := callbackIndex;
  params.sessionId := sessionId;
  params.sipSignaling := sipMessage;
...
procedure TForm1.onVideoDecoderCallback(var Message: TMessage);
var
  params: TONVideoDecoderCallbackParams;
  callbackIndex: NativeInt;
  index: integer;
begin
  ...
  params := TONVideoDecoderCallbackParams(Message.LPARAM);
...
begin
  ...
  sdkLib.joinToConference(conferenceId, Sessions[index].GetSessionID());
...
begin
  ...
  if (Initialized = true) then
  begin
    ShowMessage('alreay Initialized.');
    Exit;
  end;
...
begin
 if (Initialized = False) then
  ...
  if (pos('sip:', EditPhoneNumber.Text)=0) or (pos('@', EditPhoneNumber.Text)=0) then
  begin
  ...
  ListBoxLog.Items.Add('Line ' + IntToStr(Currentline) + ': calling...');
  ...
  6:
   begin
    ...
    begin
      ...
      begin
           ShowMessage('Can not create more than one conference at the same time.');
          Exit;
      end;
    ...
    // Before stop the conference, MUST place all lines to hold state
      ...
      sdkLib.destroyConference(conferenceId);
      conferenceId := 0;
